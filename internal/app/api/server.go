...
import (
	...
	xraymanagerapp "github.com/rebeccapanel/rebecca/internal/app/xraymanager"
...
type Server struct {
	...
	xrayManager          xraymanagerapp.Manager
}
...
func New(cfg Config) (*Server, error) {
	...
	// Initialize local Xray manager
	xrayRepo := xraymanagerapp.NewRepository(pool.DB, pool.Dialect)
	xrayCfg := xraymanagerapp.Config{}
	xrayManager := xraymanagerapp.NewManager(xrayCfg, xrayRepo, outboundSubs)
...
func (s *Server) StartBackground(ctx context.Context) {
	s.backgroundOnce.Do(func() {
		...
		go s.StartLocalXray(ctx)
	})
}

// XrayManager returns the local Xray manager.
func (s *Server) XrayManager() xraymanagerapp.Manager {
	return s.xrayManager
}

// StartLocalXray attempts to start the local Xray process. This is best-effort:
// if Xray is not installed or fails to start, the panel continues running and
// reports the error in status/API. The panel never crashes due to Xray failure.
func (s *Server) StartLocalXray(ctx context.Context) {
	if s.xrayManager == nil {
		return
	}
	status := s.xrayManager.Status()
	if !status.Installed {
		logging.Infof(logging.ComponentRuntime, "[Xray] binary not installed; skipping auto-start")
		return
	}
	logging.Infof(logging.ComponentRuntime, "[Xray] starting local xray")
	if err := s.xrayManager.Start(ctx); err != nil {
		logging.Warnf(logging.ComponentRuntime, "[Xray] auto-start failed: %v", err)
	}
}
