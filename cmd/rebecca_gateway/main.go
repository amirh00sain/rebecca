...
func main() {
	...
	select {
	case <-ctx.Done():
		// Gracefully stop the local Xray process before shutting down the HTTP server
		if xrayMgr := api.XrayManager(); xrayMgr != nil {
			logging.Infof(logging.ComponentRuntime, "[Xray] shutting down local xray")
			if err := xrayMgr.Stop(); err != nil {
				logging.Warnf(logging.ComponentRuntime, "[Xray] shutdown failed: %v", err)
			}
		}
